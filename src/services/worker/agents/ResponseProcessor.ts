...
export async function processAgentResponse(
  ...
  if (!parsed.valid) {
    ...
    // and avoid pointless retry loops on the federation pool.
    const isIntentionalEmpty = /no\s+observations?\s+to\s+record|nothing\s+to\s+record|no\s+(?:tool|technical)\s+(?:executions?|changes?)/i.test(text);
