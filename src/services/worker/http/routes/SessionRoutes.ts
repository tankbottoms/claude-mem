...
export class SessionRoutes extends BaseRouteHandler {
  ...
  // (#1590, #2127, #2098). Upstream uses 24h. The federation fork extends
  // to 30d so we can retry historical-failure backfills against the same
  // session_db_id without it being killed mid-replay; this is safe in our
  // setup because the worker pool is local and there are no per-session
  // API costs with the federation pool models.
  private static readonly MAX_SESSION_WALL_CLOCK_MS = 30 * 24 * 60 * 60 * 1000; // 30 days (fork override of upstream #1590)
