"use strict";var ut=Object.create;var x=Object.defineProperty;var mt=Object.getOwnPropertyDescriptor;var lt=Object.getOwnPropertyNames;var Et=Object.getPrototypeOf,gt=Object.prototype.hasOwnProperty;var Tt=(r,e)=>{for(var t in e)x(r,t,{get:e[t],enumerable:!0})},ae=(r,e,t,s)=>{if(e&&typeof e=="object"||typeof e=="function")for(let n of lt(e))!gt.call(r,n)&&n!==t&&x(r,n,{get:()=>e[n],enumerable:!(s=mt(e,n))||s.enumerable});return r};var M=(r,e,t)=>(t=r!=null?ut(Et(r)):{},ae(e||!r||!r.__esModule?x(t,"default",{value:r,enumerable:!0}):t,r)),ft=r=>ae(x({},"__esModule",{value:!0}),r);var Ut={};Tt(Ut,{generateContext:()=>ie});module.exports=ft(Ut);var dt=M(require("path"),1),ct=require("os"),pt=require("fs");var Ee=require("bun:sqlite");var h=require("path"),V=require("os"),U=require("fs");var pe=require("url");var A=require("fs"),L=require("path"),ce=require("os"),Y=(o=>(o[o.DEBUG=0]="DEBUG",o[o.INFO=1]="INFO",o[o.WARN=2]="WARN",o[o.ERROR=3]="ERROR",o[o.SILENT=4]="SILENT",o))(Y||{}),de=(0,L.join)((0,ce.homedir)(),".claude-mem"),q=class{level=null;useColor;logFilePath=null;logFileInitialized=!1;constructor(){this.useColor=process.stdout.isTTY??!1}ensureLogFileInitialized(){if(!this.logFileInitialized){this.logFileInitialized=!0;try{let e=(0,L.join)(de,"logs");(0,A.existsSync)(e)||(0,A.mkdirSync)(e,{recursive:!0});let t=new Date().toISOString().split("T")[0];this.logFilePath=(0,L.join)(e,`claude-mem-${t}.log`)}catch(e){console.error("[LOGGER] Failed to initialize log file:",e),this.logFilePath=null}}}getLevel(){if(this.level===null)try{let e=(0,L.join)(de,"settings.json");if((0,A.existsSync)(e)){let t=(0,A.readFileSync)(e,"utf-8"),n=(JSON.parse(t).CLAUDE_MEM_LOG_LEVEL||"INFO").toUpperCase();this.level=Y[n]??1}else this.level=1}catch{this.level=1}return this.level}correlationId(e,t){return`obs-${e}-${t}`}sessionId(e){return`session-${e}`}formatData(e){if(e==null)return"";if(typeof e=="string")return e;if(typeof e=="number"||typeof e=="boolean")return e.toString();if(typeof e=="object"){if(e instanceof Error)return this.getLevel()===0?`${e.message}
...
`+JSON.stringify(o,null,2):c=" "+this.formatData(o));let m="";if(n){let{sessionId:E,memorySessionId:T,correlationId:b,...g}=n;Object.keys(g).length>0&&(m=` {${Object.entries(g).map(([f,O])=>`${f}=${O}`).join(", ")}}`)}let l=`[${i}] [${a}] [${d}] ${p}${s}${m}${c}`;if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,l+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),c=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",m={...s,location:c};return this.warn(e,`[HAPPY-PATH] ${t}`,m,n),o}},u=new q;var Ot={};function St(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,pe.fileURLToPath)(Ot.url))}var ht=St();function bt(){if(process.env.CLAUDE_MEM_DATA_DIR)return process.env.CLAUDE_MEM_DATA_DIR;let r=(0,h.join)((0,V.homedir)(),".claude-mem"),e=(0,h.join)(r,"settings.json");try{if((0,U.existsSync)(e)){let{readFileSync:t}=require("fs"),s=JSON.parse(t(e,"utf-8")),n=s.env??s;if(n.CLAUDE_MEM_DATA_DIR)return n.CLAUDE_MEM_DATA_DIR}}catch{}return r}var C=bt(),I=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,V.homedir)(),".claude"),Pt=(0,h.join)(I,"plugins","marketplaces","thedotmack"),Xt=(0,h.join)(C,"archives"),jt=(0,h.join)(C,"logs"),Gt=(0,h.join)(C,"trash"),Ht=(0,h.join)(C,"backups"),Bt=(0,h.join)(C,"modes"),Wt=(0,h.join)(C,"settings.json"),_e=(0,h.join)(C,"claude-mem.db"),Yt=(0,h.join)(C,"vector-db"),qt=(0,h.join)(C,"observer-sessions"),Vt=(0,h.join)(I,"settings.json"),Kt=(0,h.join)(I,"commands"),Jt=(0,h.join)(I,"CLAUDE.md");function ue(r){(0,U.mkdirSync)(r,{recursive:!0})}function me(){return(0,h.join)(ht,"..")}var le=require("crypto");var Rt=3e4;function k(r,e,t){return(0,le.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function w(r,e,t){let s=t-Rt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=_e){e!==":memory:"&&ue(C),this.db=new Ee.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,p=s==="date_asc"?"ASC":"DESC",c=n?`LIMIT ${n}`:"",m=e.map(()=>"?").join(","),l=[...e],E=[];if(o&&(E.push("project = ?"),l.push(o)),i)if(Array.isArray(i)){let g=i.map(()=>"?").join(",");E.push(`type IN (${g})`),l.push(...i)}else E.push("type = ?"),l.push(i);if(a){let g=Array.isArray(a)?a:[a],S=g.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");l.push(...g),E.push(`(${S.join(" OR ")})`)}if(d){let g=Array.isArray(d)?d:[d],S=g.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");g.forEach(f=>{l.push(`%${f}%`,`%${f}%`)}),E.push(`(${S.join(" OR ")})`)}let T=E.length>0?`WHERE id IN (${m}) AND ${E.join(" AND ")}`:`WHERE id IN (${m})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${T}
    ...
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,p,d,a);return{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),c=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(c.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),p=new Date(d).toISOString();return this.db.transaction(()=>{let m=[],l=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let T of s){let b=k(e,T.title,T.narrative),g=w(this.db,b,d);if(g){m.push(g.id);continue}let S=l.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,b,p,d);m.push(Number(S.lastInsertRowid))}let E=null;if(n){let b=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,p,d);E=Number(b.lastInsertRowid)}return{observationIds:m,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,p){let c=p??Date.now(),m=new Date(c).toISOString();return this.db.transaction(()=>{let E=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let f=k(e,S.title,S.narrative),O=w(this.db,f,c);if(O){E.push(O.id);continue}let _t=T.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),a||null,d,f,m,c);E.push(Number(_t.lastInsertRowid))}let b;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,m,c);b=Number(f.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(c,o),{observationIds:E,summaryId:b,createdAtEpoch:c}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),p=[...e],c=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&p.push(o),this.db.prepare(`
