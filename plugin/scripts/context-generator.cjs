...
`+JSON.stringify(o,null,2):c=" "+this.formatData(o));let l="";if(n){let{sessionId:E,memorySessionId:T,correlationId:S,...g}=n;Object.keys(g).length>0&&(l=` {${Object.entries(g).map(([f,O])=>`${f}=${O}`).join(", ")}}`)}let _=`[${i}] [${a}] [${d}] ${p}${s}${l}${c}`;if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,_+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),c=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",l={...s,location:c};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},u=new q;var Ot={};function ht(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,pe.fileURLToPath)(Ot.url))}var bt=ht();function St(){if(process.env.CLAUDE_MEM_DATA_DIR)return process.env.CLAUDE_MEM_DATA_DIR;let r=(0,b.join)((0,V.homedir)(),".claude-mem"),e=(0,b.join)(r,"settings.json");try{if((0,U.existsSync)(e)){let{readFileSync:t}=require("fs"),s=JSON.parse(t(e,"utf-8")),n=s.env??s;if(n.CLAUDE_MEM_DATA_DIR)return n.CLAUDE_MEM_DATA_DIR}}catch{}return r}var N=St(),I=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,V.homedir)(),".claude"),Pt=(0,b.join)(I,"plugins","marketplaces","thedotmack"),Xt=(0,b.join)(N,"archives"),jt=(0,b.join)(N,"logs"),Gt=(0,b.join)(N,"trash"),Ht=(0,b.join)(N,"backups"),Bt=(0,b.join)(N,"modes"),Wt=(0,b.join)(N,"settings.json"),me=(0,b.join)(N,"claude-mem.db"),Yt=(0,b.join)(N,"vector-db"),qt=(0,b.join)(N,"observer-sessions"),Vt=(0,b.join)(I,"settings.json"),Kt=(0,b.join)(I,"commands"),Jt=(0,b.join)(I,"CLAUDE.md");function ue(r){(0,U.mkdirSync)(r,{recursive:!0})}function _e(){return(0,b.join)(bt,"..")}var le=require("crypto");var Rt=3e4;function k(r,e,t){return(0,le.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function w(r,e,t){let s=t-Rt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=me){e!==":memory:"&&ue(N),this.db=new Ee.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,type:i,concepts:a,files:d}=t,p=s==="date_asc"?"ASC":"DESC",c=n?`LIMIT ${n}`:"",l=e.map(()=>"?").join(","),_=[...e],E=[];if(o&&(E.push("project = ?"),_.push(o)),i)if(Array.isArray(i)){let g=i.map(()=>"?").join(",");E.push(`type IN (${g})`),_.push(...i)}else E.push("type = ?"),_.push(i);if(a){let g=Array.isArray(a)?a:[a],h=g.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");_.push(...g),E.push(`(${h.join(" OR ")})`)}if(d){let g=Array.isArray(d)?d:[d],h=g.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");g.forEach(f=>{_.push(`%${f}%`,`%${f}%`)}),E.push(`(${h.join(" OR ")})`)}let T=E.length>0?`WHERE id IN (${l}) AND ${E.join(" AND ")}`:`WHERE id IN (${l})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${T}
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),p=k(e,s.title,s.narrative),c=w(this.db,p,a);if(c)return{id:c.id,createdAtEpoch:c.created_at_epoch};let _=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,p,d,a);return{id:Number(_.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),c=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(c.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),p=new Date(d).toISOString();return this.db.transaction(()=>{let l=[],_=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let T of s){let S=k(e,T.title,T.narrative),g=w(this.db,S,d);if(g){l.push(g.id);continue}let h=_.run(e,t,T.type,T.title,T.subtitle,JSON.stringify(T.facts),T.narrative,JSON.stringify(T.concepts),JSON.stringify(T.files_read),JSON.stringify(T.files_modified),o||null,i,S,p,d);l.push(Number(h.lastInsertRowid))}let E=null;if(n){let S=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,p,d);E=Number(S.lastInsertRowid)}return{observationIds:l,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,p){let c=p??Date.now(),l=new Date(c).toISOString();return this.db.transaction(()=>{let E=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let h of s){let f=k(e,h.title,h.narrative),O=w(this.db,f,c);if(O){E.push(O.id);continue}let mt=T.run(e,t,h.type,h.title,h.subtitle,JSON.stringify(h.facts),h.narrative,JSON.stringify(h.concepts),JSON.stringify(h.files_read),JSON.stringify(h.files_modified),a||null,d,f,l,c);E.push(Number(mt.lastInsertRowid))}let S;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,l,c);S=Number(f.lastInsertRowid)}return this.db.prepare(`
